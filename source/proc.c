...
/*int findProcess(char* procName) {
	int procPID = 0;
	printf_debug("before while loop, procPID = %d, %s", procPID, " placeholder\n");
	while (!procPID) {
		int mib[3];
		size_t len;
		mib[0] = CTL_KERN;
		mib[1] = KERN_PROC;
		mib[2] = KERN_PROC_ALL;
		printf_debug("before sysctl, procPID = %d, %s", procPID, " placeholder\n");

		if (sysctl(mib, 3, NULL, &len, NULL, 0) != -1) {
			if (len > 0) {
				printf_debug("in first if statement, before malloc, procPID = %d, %s", procPID, " placeholder\n");
				void* dump = malloc(len);
				printf_debug("after malloc, procPID = %d, %s", procPID, " placeholder\n");
				if (sysctl(mib, 3, dump, &len, NULL, 0) != -1) {
					printf_debug("sysctl if check passed, procPID = %d, %s", procPID, " placeholder\n");
